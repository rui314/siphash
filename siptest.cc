// Based on test.c of the reference implementation.
//
// This is a test file. Actual implementation is in siphash.h.
...
int main() {
  ...
  for (int i = 0; i < 64; i++) {
    ...
    int off = 0;
    for (int j = 1; off + j < i; off += j, j++)
      hasher.update(in + off, j);

    hasher.update(in + off, i - off);
    hasher.finish(out);

    if (memcmp(out, &vectors_sip64[i], 8)) {
      printf("SipHash: fail for %d bytes\n", i);
      fails++;
    }
  }
  ...
  for (int i = 0; i < 64; i++) {
    ...
    if (memcmp(out, &vectors_sip128[i], 16)) {
      printf("SipHash128: fail for %d bytes\n", i);
      fails++;
    }
  }
